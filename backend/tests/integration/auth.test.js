...
const { user_one_access_token, admin_access_token } = require('../fixtures/token.fixture');
...
describe('Auth routes', () => {
  describe('POST /v1/auth/register', () => {
    ...
    test('should returrn 400 error if email is invalid', async () => {
  ...
  describe('POST /v1/auth/logout', () => {
    test('should return 204 if refresh token is valid', async () => {
      ...
      expect(dbRefreshTokenDoc).toBeNull();
    });
  ...
  describe('POST /v1/auth/refresh-tokens', () => {
    ...
    test('should return 401 error if refresh token is signed using an invalid secret', async () => {
      await insertUsers([userOne]);
      const expires = moment().add(config.jwt.refreshExpirationDays, 'days');
      const refreshToken = tokenService.generateToken(userOne._id, expires, tokenTypes.REFRESH, 'invalid secret');
      await tokenService.saveToken(refreshToken, userOne._id, expires, tokenTypes.REFRESH);
    ...
    test('should return 401 error if refresh token is not blacklisted', async () => {
      await insertUsers([userOne]);
      const expires = moment().add(config.jwt.refreshExpirationDays, 'days');
      const refreshToken = tokenService.generateToken(userOne._id, expires, tokenTypes.REFRESH);
      await tokenService.saveToken(refreshToken, userOne._id, expires, tokenTypes.REFRESH, true);
  ...
  describe('POST /v1/auth/forgot-password', () => {
    ...
    test('should return 204 and send reseet password email to the user', async () => {
      await insertUsers([userOne]);
  ...
  describe('POST /v1/auth/reset-password', () => {
    test('should return 204 and reeset the password', async () => {
      await insertUsers([userOne]);
  ...
  describe('POST /v1/auth/send-verification-email', () => {
    ...
    test('should return 204 and send verification email to the user', async () => {
      ...
      await request(app)
        ...
        .set('Authorization', `Bearer ${user_one_access_token}`)
        .expect(httpStatus.NO_CONTENT);

      expect(sendVerificationEmailSpy).toHaveBeenCalledWith(userOne.email, expect.any(String));

      const verifyEmailToken = sendVerificationEmailSpy.mock.calls[0][1];
      const dbVerfiyEmailToken = await Token.findOne({ token: verifyEmailToken, user: userOne._id });

      expect(dbVerfiyEmailToken).toBeDefined();
    });
  ...
  describe('POST /v1/auth/verify-email', () => {
    ...
    test('should return 401 if verify email token is blacklisted', async () => {
      ...
      const expires = moment().add(config.jwt.verifyEmailExpirationMinutes, 'minutes');
      const verifyEmailToken = tokenService.generateToken(userOne._id, expires, tokenTypes.VERIFY_EMAIL);
      await tokenService.saveToken(verifyEmailToken, userOne._id, expires, tokenTypes.RESET_PASSWORD, true);
...
describe('Auth middleware', () => {
  test('should call next with no errors if access token is valid', async () => {
    await insertUsers([userOne]);
    const req = httpMocks.createRequest({ headers: { authorization: `Bearer ${user_one_access_token}` } });
    const next = jest.fn();
  ...
  test('should next with unauthorized error if access token is not a valid jwt token', async () => {
    await insertUsers([userOne]);
    const req = httpMocks.createRequest({ headers: { authorization: `Bearer randomTokenHaha` } });
    const next = jest.fn();
  ...
  test('should call next with unauthorized error if the access token is generated with an invalid secret', async () => {
    await insertUsers([userOne]);
    const expires = moment().add(config.jwt.accessExpirationMinutes, 'minutes');
    const accessToken = tokenService.generateToken(userOne._id, expires, tokenTypes.ACCESS, 'invalid secret');
  ...
  test('should call next with unauthorized error if the access token is expired', async () => {
    await insertUsers([userOne]);
    const expires = moment().subtract(1, 'minutes');
  ...
  test('should call next with no error if user does not have required rights but userId is in params', async () => {
    await insertUsers([userOne]);
    const req = httpMocks.createRequest({
      headers: { authorization: `Bearer ${user_one_access_token}` },
  ...
  test('should call next with no error if user has required rights', async () => {
